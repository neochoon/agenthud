...
describe("discoverSessions", () => {
  ...
  it("discovers a top-level session with no sub-agents", () => {
    ...
    vi.mocked(readdirSync).mockImplementation((p) => {
      ...
      if (path === projectDir) return ["abc123.jsonl"] as unknown as ReturnType<typeof readdirSync>;
    ...
    vi.mocked(statSync).mockImplementation((p) => {
      ...
      return { isDirectory: () => isDir, mtimeMs: NOW - 10_000, size: 1000 } as ReturnType<typeof statSync>;
    });
  ...
  it("marks session as running when mtime is within 30s", () => {
    ...
    vi.mocked(readdirSync).mockImplementation((p) => {
      const path = String(p);
      if (path === projectsDir) return ["-Users-neo-myproject"] as unknown as ReturnType<typeof readdirSync>;
      if (path === projectDir) return ["sess1.jsonl"] as unknown as ReturnType<typeof readdirSync>;
      return [] as unknown as ReturnType<typeof readdirSync>;
    });
    vi.mocked(statSync).mockImplementation((p) => {
      const path = String(p);
  ...
  it("nests sub-agents under their parent", () => {
    ...
    vi.mocked(existsSync).mockImplementation((p) => {
      const path = String(p);
      return [projectsDir, projectDir, subagentsDir].includes(path) || path.endsWith(".jsonl");
    });
    vi.mocked(readdirSync).mockImplementation((p) => {
      const path = String(p);
      if (path === projectsDir) return ["-Users-neo-proj"] as unknown as ReturnType<typeof readdirSync>;
      if (path === projectDir) return ["parent-id.jsonl", "parent-id"] as unknown as ReturnType<typeof readdirSync>;
      if (path === subagentsDir) return ["child-id.jsonl"] as unknown as ReturnType<typeof readdirSync>;
      return [] as unknown as ReturnType<typeof readdirSync>;
    });
    vi.mocked(statSync).mockImplementation((p) => {
      const path = String(p);
